...
def update_from_consul():
    if "consul_server" not in app.config or app.config["consul_server"] is None:
        ...
        try:
            app.config["consul_server"] = consul.Consul(
                host=app.config["CONFIG_HOST"], port=app.config["CONFIG_PORT"]
            )
            print(
                "Consul config server [{}:{}] connected! ".format(
            ...
            print(
                "Can not connect to Consul config server [{}:{}] ! ".format(
                    app.config["CONFIG_HOST"], app.config["CONFIG_PORT"]
                )
            )
    ...
    try:
        ...
        print(
            "!!!CONSUL SERVER PROBLEM [{}:{}]\n{}".format(
                app.config["CONFIG_HOST"], app.config["CONFIG_PORT"], str(e)
            )
        )
